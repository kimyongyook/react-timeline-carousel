...
class CarouselTimeline extends React.Component {
  ...
  handleOnRightBtn(){
    ...
    var boxWidth = this.contDiv.current.offsetWidth;
    this.setMoveMotion()
    var setTmp = this.mouseState.currentX - 500;
    this.mouseState.currentX = setTmp < -boxWidth ? -boxWidth : setTmp;
    this.moveTimeline(this.mouseState.currentX);
  }
  handleOnLeftBtn(){
    console.log("handleOnLeftBtn");
    this.setMoveMotion()
    var setTmp = this.mouseState.currentX + 500;
    this.mouseState.currentX = setTmp > 0 ? 0 : setTmp;
    this.moveTimeline(this.mouseState.currentX);
  }

  setMoveMotion(){
    this.tlDiv.current.style.transition="transform 0.5s ease 0s";
  }
  unsetMoveMotion(){
    this.tlDiv.current.style.transition="transform 0s ease 0s";
  }
  moveTimeline(_x){
